...
class StewartPlatform:
    def __init__(self, id, r_B, r_P, gamma_B_deg, gamma_P_deg, serial_port=None, control_parameters=None, dt=0.01,
                 controller_type="PID", base_offset=None, home_position=None, home_orientation=None):

        self.platform_id = id
        self.dt = dt
        self.controller_type = controller_type  # "PID" or "DSTA"
        self.servo_arm_length = 2.37        # mm

        # PID Control gains
        self.kp = [100]*6  # Individual Proportional gains for each motor
        self.ki = [5]*6  # Individual Integral gains for each motor
        self.kd = [50]*6  # Individual Derivative gains for each motor

        self.previous_errors = np.zeros(6)
        self.integral_errors = np.zeros(6)  # To store cumulative errors for integral control
        self.current_angles = np.zeros(6)   # Current servo angles
        self.serial_port = serial.Serial(serial_port, 250000) if serial_port else None

        # Set home position and orientation
        self.home_position = np.array(home_position) if home_position else np.array([0, 0, 15.2])  # (x,y,z) Translation in mm
    ...
    def inverse_kinematics(self, position, orientation):
        """
        Compute the inverse kinematics of a Stewart Platform using linear actuators
        ...
        :return: angles: (ndarray) Servos angle (6x1)
        """
        ...
        for i in range(6):
            vector_b_p = P_global[i] - self.base_points[i]  # Vector from base to platform
            distance = np.linalg.norm(vector_b_p)           # Compute distance fromm base to platform

            # Calculate servo angles using Law of Cosines
            angle_rad = np.arccos(distance/(2*self.servo_arm_length))
            angle_deg = np.degrees(angle_rad)

            servo_angles[i] = angle_deg

        return servo_angles

    def pid_control(self, desired_angle, servo_idx):
        """
        ...
        error = desired_angle - self.current_angles[servo_idx]

        # Proportional component
        P = self.kp[servo_idx] * error

        # Integral Component
        self.integral_errors[servo_idx] += error * self.dt
        I = self.ki[servo_idx] * self.integral_errors[servo_idx]

        # Derivative component
        D = self.kd[servo_idx] * ((error - self.previous_errors[servo_idx]) / self.dt)  # Derivative component

        # Compute total control signal
        control_signals = P + I + D  # Control signals

        # Update previous error
        self.previous_errors[servo_idx] = error

        # Clamp control signals within actuator limits
        new_angle = self.current_lengths[servo_idx] + control_signals * self.dt
        return new_angle - self.current_angles[servo_idx]
        ...
        for step in range(steps):
            ...
            for i in range(6):
                ...
                if self.controller_type == 'DSTA':
                    control_signals[i] = self.dsta_controllers[i].derivative(
                        errors[i])  # Individual actuator control update
...
class StewartPlatform:
    def __init__(self, id, r_B, r_P, gamma_B_deg, gamma_P_deg, serial_port=None, control_parameters=None, dt=0.01,
        ...
        self.home_orientation = np.array(home_orientation) if home_orientation else np.array(
